...
def menuMedico(usuarito, contrasenyita, opcion):
    ...
    if opcion == 2:
        ...
        try:
            ...
            SQLita = f"SELECT fecha_entrada, fecha_salida FROM diagnosticos WHERE p_id = (SELECT p_id FROM personal WHERE dni = '{dni_medico}') AND fecha_entrada >= current_timestamp ORDER BY fecha_entrada DESC;"
            SQLita2 = f"SELECT fecha_entrada, fecha_salida FROM operacion WHERE p_id = (SELECT p_id FROM personal WHERE dni = '{dni_medico}') AND fecha_entrada >= current_timestamp ORDER BY fecha_entrada DESC;"
